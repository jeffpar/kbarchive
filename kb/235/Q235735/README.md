...
## Q235735: SMS: Scheduled Site Backup Fails to Run Daily
	...
	When the Site Maintenance Task is scheduled to run daily, it fails to run. The
	minimum time interval at which the Backup Task will run is weekly, starting on
	the next available day (from when it was set) within the time parameters
	specified.
