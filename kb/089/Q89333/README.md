...
## Q89333: Minimum System Requirements for Windows for Workgroups
	...
	Below are the minimum system requirements for Microsoft Windows for Workgroups.
	Requirements differ for a client (a machine that is not sharing its
	printers/directories) and a server (a machine that is sharing its
	printers/directories).
	...
	- 3 megabytes (MB) total memory for version 3.1 (2 MB with no network
	  installed)
	...
	If the server is also acting as a Mail postoffice, additional hard drive space is
	required on the server.
	
	Additional query words: 3.10 3.11 minimum setup
