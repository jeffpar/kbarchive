...
## Q231855: Wordpad.exe May Delete a File With Read Only Permissions
	...
	When you attempt to save a document in Wordpad using the Save as command, the
	file may be deleted without being saved if the current user only has read-only
	permissions to the file.
	
	CAUSE
	=====
	
	This problem occurs because the exception handler that was registered for the
	open dialog box was inadvertently handling the exception by deleting the file.
