...
{% raw %}
	...
	When a user's mailbox is located on an Exchange Server 5.5 computer and is a
	member of a distribution list (DL) that is based on a computer that is running
	Microsoft Exchange 2000 Server, the Exchange Server 5.5 user may receive
	duplicate e-mail messages if a message is sent to the DL and the user is carbon
	copied (cc) on the e-mail message.
