...
## Q314220: Microsoft Visual C# .NET Step by Step Comments and Corrections
	...
	relating to the Microsoft Press book Microsoft Visual C# .NET Step by Step, ISBN
	0-7356-1289-7.
	...
	- Page 275: "namespace Windows.Drawing" Should Be "namespace
	  System.Windows.Drawing"
	
	- Page 282: Incorrect Use of Vertical Ellipses In Code Sample
	
	- Page 292: "Declare a read-only property" Should Be "Declare a read-only
	  indexer"
	...
	Page xi includes Windows XP Home Edition as part of the minimum system
	requirements. Windows XP Home does not include IIS, so does not meet the minimum
	requirements for this book. Windows XP Home Edition should be removed from page
	xi.
	...
	Change:
	"If the 3 binds to the * operator, the result of the multiplication (3 * 4) forms
	the right hand operand of the / operator and the result of the whole expression
	is 2 / 12."
	
	To:
	"If the 3 binds to the * operator, the result of the multiplication (3 * 2) forms
	the right hand operand of the / operator and the result of the whole expression
	4 / (3 * 2), which is 0 (again integer division)."
	...
	"As before, verify that the readDouble method you have declared compiles without
	error."
	
	To:
	"As before, verify that the readInt method you have declared compiles without
	error."
	...
	"Other statements in the method that come afterwards can use it when the method
	has finished, even though the variable disappears."
	
	To:
	"The variable is created at the statement that defines it, and other statements
	in the method that come afterwards can use it. Once the method has finished,
	though, the variable disappears."
	...
	To:
	"... is nevertheless static. However, for reasons beyond the scope of this book,
	you can only declare const field when the field is an enum, a primitive type, or
	...
	Change:
	"However, if you try this syntax, you'll get a compiletime error. Instead, you
	need to use the syntax object o = 42; where o refers to a boxed int containing
	the value 42. Here are examples:"
	...
	The arrow leading to the "42" box should come from a square "@" box with the
	label "object o" (exactly like the box at the bottom of page 159).
	
	The arrow leading out of the "42" box should lead to a square "42" box with the
	label "int i" (exactly like the the box at the bottom of page 159).
	
	
	Pages 184 & 185: Lines Incorrectly Formatted As Code
	----------------------------------------------------
	
	The last line on page 184, and the first two lines on 185 are incorrectly
	formatted as code. These lines are not part of the code sample at the bottom of
	page 184.
	...
	The image on page 199 shows 12 cards per player. This image should show 13 cards
	per player.
	...
	On page 282, the code sample incorrectly uses vertical ellipses. Change these two
	instances to horizontal ellipses (...).
	...
	On page 319, there is an error at the bottom of the Struct vs. Class box. The
	last line, "This is for efficiency reasons...", should be deleted entirely.
	...
	The image on page 333 is incorrect. It should show an empty project window with
	the Bell Ringers logo background.
	...
	There is an error in the table at the bottom of page 512. The Properties for
	Label 7 should be changed from:
	...
	The "important" box on page 539 needs to have the following text added to the
	end:
	
	"If you have installed and configured the QuickStart samples, the account used by
	ASP.NET will already have been granted access to the Northwind database. If you
	have not already installed the QuickStart samples you can grant access to the
	ASP.NET service using the steps shown below."
	...
	Additional query words: 0-7356-1289-7 SHARP JAGGER EUBOOK EUSBS VSTUDIO .NET VC
