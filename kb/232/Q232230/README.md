...
{% raw %}
	...
	Print jobs may intermittently not be printed when a print server is busy. When
	this occurs, event ID 10 is recorded in the event log with a message stating "0
	bytes, 0 pages printed." Event ID 13 indicates that the print job has been
	deleted.
	
	CAUSE
	=====
	
	There is a problem with the local procedure call (LPC) code that can cause
	messages to be lost when the client and the server are the same process ID (PID)
