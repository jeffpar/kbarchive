...
{% raw %}
	...
	When you use the Administrator program (Admin.exe) to modify the configuration
	of an external postoffice, Admin.exe lists all the postoffices that are
	currently defined. If the network contains more than 200 postoffices, Admin.exe
	may not display the postoffices properly as you scroll up or down the list.
