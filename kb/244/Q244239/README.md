...
## Q244239: Stop 0xA in Netbt.sys When Using Third-Party Servers
	...
	A computer running Windows NT may display an error message on a blue screen with
	the following values:
	
	  stop 0x0000000a (0x00000002,0x00000000,0xf31c8485)
	  NETBT.SYS
	
	The third value may vary, depending where Netbt.sys is loaded in memory.
	
	CAUSE
	=====
	
	In certain error message situations, the Windows NT redirector (Rdr.sys) returns
	that it has read all the data sent by the transport even though it has not. This
	typically happens when you are using certain third-party servers. The NetBT
	transport driver is not handling this situation properly.
	...
	attributes or later:
	  ...
	  -------------------------------------------------
	  10/18/99   8:45p   123,152   Netbt.sys   Intel
	  10/18/99   8:44p   184,320   Netbt.sys   Alpha
