---
layout: page
title: "Q261019: MCM2: Textures Become Distorted If You Receive a Reminder"
permalink: kb/261/Q261019/
---

## Q261019: MCM2: Textures Become Distorted If You Receive a Reminder

	Article: Q261019
	Product(s): Microsoft Home Games
	Version(s): 2.0
	Operating System(s): 
	Keyword(s): kb3rdparty kbdisplay kbimu msgamekbfaq
	Last Modified: 12-JUN-2001
	
	-------------------------------------------------------------------------------
	The information in this article applies to:
	
	- Microsoft Motocross Madness 2, version 2.0 
	-------------------------------------------------------------------------------
	
	SUMMARY
	=======
	
	If you receive a reminder or a warning message from another program that is
	running on your computer while you play Microsoft Motocross Madness 2, textures
	in the game may become distorted.
	
	MORE INFORMATION
	================
	
	If an anti-virus or a calendar program minimizes Motocross Madness 2 to display
	a reminder or a warning message, textures in the game may become distorted and
	appear white when you restore the game.
	
	If you encounter this issue, quit unnecessary programs, and then quit and restart
	Motocross Madness 2.
	
	To quit all unnecessary programs, use the appropriate method for your version of
	Microsoft Windows.
	
	Microsoft Windows 95 or Microsoft Windows 98:
	
	1. Press CTRL+ALT+DELETE.
	
	2. In the Close Program dialog box, click any program except Explorer or Systray
	  (which are components of Microsoft Windows), and then click End Task.
	
	  If you receive a message stating that the program is busy or not responding,
	  click End Task again.
	
	3. Repeat steps 1 and 2 to quit all unnecessary programs that are running on
	  your computer.
	
	Microsoft Windows 2000:
	
	1. Press CTRL+SHIFT+ESC.
	
	2. On the Applications tab, click any program, and then click End Task.
	
	  Repeat this step to quit all unnecessary programs that are running on your
	  computer.
	
	3. Close Windows Task Manager.
	
	NOTE: Some programs start automatically every time you restart your computer. For
	information about how to disable these programs, see the printed or online
	documentation for the program.
	
	Additional query words: 2.00 msgame mcm2 motorcross corrupt display task switch popup
	
	======================================================================
	Keywords          : kb3rdparty kbdisplay kbimu msgame kbfaq
	Technology        : kbGamesSearch kbMotocrossSearch kbMotocrossM2
	Version           : :2.0
	Issue type        : kbprb
	
	=============================================================================
	
