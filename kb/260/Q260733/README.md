---
layout: page
title: "Q260733: StarLancer: Desktop Icons Appear on the Startup Screen"
permalink: /kb/260/Q260733/
---

## Q260733: StarLancer: Desktop Icons Appear on the Startup Screen

	Article: Q260733
	Product(s): Microsoft Home Games
	Version(s): 1.0
	Operating System(s): 
	Keyword(s): kb3rdparty kbdisplay kbimu msgame
	Last Modified: 12-NOV-2001
	
	-------------------------------------------------------------------------------
	The information in this article applies to:
	
	- Microsoft StarLancer, version 1.0 
	-------------------------------------------------------------------------------
	
	SYMPTOMS
	========
	
	When you play Microsoft StarLancer, desktop icons may appear on the StarLancer
	startup screen.
	
	CAUSE
	=====
	
	This behavior can occur if the following conditions are true:
	
	- A Creative Labs Graphics Blaster Exxtreme video adapter is installed in your
	  computer.
	
	- The screen resolution in the game is set to 1024 x 768 pixels.
	
	RESOLUTION
	==========
	
	To resolve this issue, download and install the latest version of the video
	driver for your video adapter from the following Creative Labs Web site:
	
	  http://support.soundblaster.com
	
	If the issue continues to occur, download and install the latest version of the
	Permedia 2 reference driver from the following 3DLabs Web site:
	
	  http://www.3dlabs.com
	
	MORE INFORMATION
	================
	
	To work around this issue, reduce the screen resolution in the game to 800 x 600
	pixels.
	
	The third-party contact information included in this article is provided to help
	you find the technical support you need. This contact information is subject to
	change without notice. Microsoft in no way guarantees the accuracy of this
	third-party contact information.
	
	The third-party products discussed in this article are manufactured by vendors
	independent of Microsoft; we make no warranty, implied or otherwise, regarding
	these products' performance or reliability.
	
	Additional query words: 1.00 msgame star lancer
	
	======================================================================
	Keywords          : kb3rdparty kbdisplay kbimu msgame 
	Technology        : _IKkbbogus kbGamesSearch kbStarlancerSearch kbStarlancer
	Version           : :1.0
	Issue type        : kbprb
	
	=============================================================================
	
