---
layout: page
title: "Q136022: Mac Bookshelf 95: Israel Anthem Suddenly Stops"
permalink: /kb/136/Q136022/
---

## Q136022: Mac Bookshelf 95: Israel Anthem Suddenly Stops

{% raw %}

	Article: Q136022
	Product(s): Microsoft Home Multimedia Titles
	Version(s): 1995 edition
	Operating System(s): 
	Keyword(s): 
	Last Modified: 13-DEC-1999
	
	-------------------------------------------------------------------------------
	The information in this article applies to:
	
	- Microsoft Bookshelf for the Macintosh 1995 edition 
	-------------------------------------------------------------------------------
	
	SYMPTOMS
	========
	
	The Israel Anthem in Bookshelf stops when the anthem nears the end of the audio
	clip.
	
	STATUS
	======
	
	Microsoft has confirmed this to be a problem in the 1995 edition of Bookshelf
	Macintosh. We are researching this problem and will post new information here in
	the Microsoft Knowledge Base as it becomes available.
	
	MORE INFORMATION
	================
	
	There is no resolution to this problem. Although the sudden stop occurs, the
	problem does not cause any problem with the system.
	
	Additional query words: mac 1995 multi media multimedia multi-media mmtitles kbmm macmm quits halts hangs hanging hanged crashed crash crashes freeze frozen locked locked-up lock hang bombed bomb down halting silent
	
	======================================================================
	Keywords          :  
	Technology        : kbHWMAC kbOSMAC kbHomeMMsearch kbBookshelfSearch kbBookShelf1995Mac
	Version           : :1995 edition
	
	=============================================================================
	

{% endraw %}
